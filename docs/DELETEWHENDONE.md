...
**What this task means:** This is a PLANNING task, not a coding task. You are creating a design document that explains how you will organize data in Redis. Think of it like writing a blueprint before building a house.

**Objective:** Create a design document that explains how Redis keys will be structured and how long data should stay in Redis (TTL = Time To Live).

**Steps:**

**Step 1: Create a new file called `docs/redis-design.md`**
- File location: `/home/lex/Documents/PersonalProjects/Lex-Shortlink-API/docs/redis-design.md`
- This is a markdown file where you will document your Redis design
- You are NOT writing code here - you are writing documentation/planning

**Step 2: Add the Redis key naming convention to the file**
- In the file, write down the pattern you will use for naming Redis keys
- Pattern: `{feature}:{identifier}:{attribute}`
- Examples to document:
  - `link:slug:abc123` - stores cached link data for slug "abc123"
  - `clicks:stream` - stores click events in a Redis stream
  - `rate:api_key:xyz789` - stores rate limit counters for API keys

**Step 3: Document the link cache key design**
- In `docs/redis-design.md`, write:
  - Key pattern: `link:slug:{slug}` (where {slug} is the actual slug like "abc123")
  - Data type: Redis Hash (like a dictionary with multiple fields)
  - Hash fields to store:
    - `original_url` - the long URL this slug points to
    - `expires_at` - when this link expires (if applicable)
    - `password_hash` - if the link is password protected
    - `is_deleted` - whether the link has been soft deleted
  - TTL (Time To Live): 3600 seconds (1 hour) for most links
  - For links with expiration: set TTL to match the link's expiration time minus 5 minutes

**Step 4: Document the click buffer key design**
- In `docs/redis-design.md`, write:
  - Key pattern: `clicks:stream` (single key for all clicks)
  - Data type: Redis Stream (like a log of events)
  - Each entry in the stream contains:
    - `link_id` - which link was clicked
    - `clicked_at` - when the click happened
    - `country_code` - where the click came from (GeoIP)
...
**Step 5: Document the rate limit key design**
- In `docs/redis-design.md`, write:
  - Key pattern: `rate:api_key:{key_hash}:{window}`
  - Data type: Redis String (just a number)
  - Two windows (time periods):
    - `rate:api_key:{key_hash}:1min` - counts requests in last 60 seconds, TTL = 60s
    - `rate:api_key:{key_hash}:1hour` - counts requests in last hour, TTL = 3600s
  - Example: `rate:api_key:abc123hash:1min` counts how many requests API key abc123hash made in the last minute

**Step 6: Document the analytics cache key design (optional)**
- In `docs/redis-design.md`, write:
  - Key pattern: `stats:link:{slug}:{period}`
  ...
  - TTL: 300 seconds (5 minutes) - analytics can be slightly stale

**Step 7: Document cache invalidation strategy**
- In `docs/redis-design.md`, write when to delete cached data:
...
**Step 8: Add a table summarizing all keys**
- In `docs/redis-design.md`, create a table like this:
...
- Add this line to the file:
  ...
  redis>=5.0.0,<6.0.0
  ```
- This tells Python to install the Redis library when you run `pip install -r requirements.txt`

**Acceptance Criteria (check these when done):**
- [ ] File `docs/redis-design.md` exists
- [ ] File contains key naming convention
- [ ] File contains design for link cache keys
- [ ] File contains design for click buffer keys
- [ ] File contains design for rate limit keys
- [ ] File contains cache invalidation strategy
- [ ] File contains summary table of all keys
- [ ] `requirements.txt` includes `redis>=5.0.0,<6.0.0`
